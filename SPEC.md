...
- **Each side (Home/Away):**
  ...
  - Two buttons per side: "Randomise" (fills all 5 slots automatically, drawing only from each slot's natural-fit players — unlike manual search, Randomise never produces an out-of-position squad on its own — and also generates a random team name) and "Clear" (empties the side, including the name)
